...
while epoch_idx < params_dict['task']['nb_epochs']:
    ...
    while True:
        ...
        new_log_str = f'vali_batch {int(vali_batch_idx)}: loss = {batch_vali_loss}\n'
        print(new_log_str, end='')
        log_str += new_log_str
        vali_loss += batch_vali_loss
        vali_batch_idx += 1

    new_log_str = f'epoch {epoch_idx}: mean vali_loss = {vali_loss / vali_batch_idx}\n'
    print(new_log_str, end='')
    log_str += new_log_str
    ...
    while True:
        try:
            ...
            new_log_str = f'train_batch {int(train_batch_idx)}: loss = {batch_train_loss}\n'
            print(new_log_str, end='')
            log_str += new_log_str
            ...
            break
    new_log_str = f'epoch {epoch_idx}: mean train_loss = {train_loss / train_batch_idx}\n'
    print(new_log_str, end='')
    with open(log_savepath, 'a') as log_recorder:
        log_recorder.write(log_str)
    del log_str
