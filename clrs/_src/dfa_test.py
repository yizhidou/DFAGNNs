...
def test(util_path_processer: dfa_utils.UtilPathProcessor,
    ...
    trained_model_params_dict = dfa_utils.parse_params(params_hash=trained_model_params_id,
                                                       ...
                                                       statistics_filepath=util_path_processer.num_pp_statistics_filepath(dataset_name=trained_dataset_name),
                                                       for_model_test=True)
    sample_path_processor = dfa_utils.SamplePathProcessor(sourcegraph_dir=trained_model_params_dict['sample_path_processor']['sourcegraph_dir'],
    ...
    test_sampler = dfa_samplers.DFASampler(task_name=trained_model_params_dict['task']['task_name'],
                                           sample_id_list=filter_sample_list(statistics_savepath=num_pp_statistics_filepath,
    ...
    for ckpt_filename, test_log_savepath in zip(ckpt_filename_list, test_log_savepath_list):
        ...
        #     task_name_this_batch, test_feedback_batch = next(test_feedback_generator)
            ...
            if not cur_sampled_id == sampled_id_remain:
                ...
                statistics_str = f'{cur_sampled_id}: num_pp = {num_pp}, num_cfg = {num_cfg}, max_out_degree_liveness = {max_out_degree}, max_in_degree = {max_in_degree}'
            ...
            if task_name_this_batch is not None:
                ...
                # print(statistics_str)
                # new_log_str = f'{task_name_this_batch}: precision = {test_precision}; recall = {test_recall}; F1 = {test_f1}\n'
