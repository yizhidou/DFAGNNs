...
class DFASampler(samplers.Sampler):
    ...
    def __init__(self,
        ...
        if self.sample_loader.dfa_version == 'others':
            algo = getattr(algorithms, 'dfa_others')
            spec = dfa_specs.DFASPECS['others']
            assert self.task_name == 'mix'
        elif self.sample_loader.dfa_version == 'plus':
            algo = getattr(algorithms, 'dfa_plus')
            spec = dfa_specs.DFASPECS['plus']
            assert self.task_name == 'mix'
        else:
            assert False, 'invalid dfa_version!'
    ...
    def _sample_data(self, length: Optional[int] = None, *args, **kwargs):
        ...
        if self.iterate_all:
            # print(f'dfa_samplers line 96, len of sample_id_list = {len(self.sample_id_list)}')
...
def FeedbackGenerator_all_tasks(dfa_sampler: DFASampler,
                                batch_size: int):
    while True:
        three_batches = dfa_sampler.next_all_tasks(batch_size=batch_size)
        if not three_batches:
            continue
        else:
            liveness_batch, reachability_batch, dominance_batch = three_batches
