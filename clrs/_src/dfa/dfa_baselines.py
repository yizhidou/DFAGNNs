...
class DFABaselineModel(model.Model):
    ...
    def __init__(
        ...
        self._create_net_fns(hidden_dim, encode_hints, processor_factory, use_lstm,
                             ...
                             hint_repred_mode, version_of_DFANet)
    ...
    def _create_net_fns(self, hidden_dim: int,
        ...
        def _use_net(features_list: List[_Features],
            ...
            # exit(666)
            if plus_or_others == 'plus':
                return dfa_nets.DFANet_plus(spec=self._spec,
                                          ...
                                          take_hint_as_outpt=self.take_hint_as_outpt)(features_list,
                                                                   repred,
                                                                   algorithm_index,
                                                                   return_hints,
                                                                   return_all_outputs)
            elif plus_or_others == 'others':
                return dfa_nets.DFANet_others(spec=self._spec,
                                          hidden_dim=hidden_dim,
                                          encode_hints=encode_hints,
                                          decode_hints=self.decode_hints,
                                          just_one_layer=self.just_one_layer,
    ...
    def _predict(self, params, rng_key: hk.PRNGSequence, features: _Features,
        ...
        if self._spec[algorithm_index]['trace_o'][-1] == specs.Type.MASK:
            ...
            if self.decode_hints and return_hints:
                # assert hint_preds is not None
    ...
    def get_measures(self, rng_key: hk.PRNGSequence,
        ...
        if return_hints and self.decode_hints:
            ...
            for time_step in range(trace_h_pred.shape[0]):
                ...
                if type_ == specs.Type.CATEGORICAL:
                    ...
                    truth_trace_h_i = feedback.features.trace_h.data[time_step+1]
                precision_h, recall_h, f1_h = self._calculate_measures(type=type_,
                                                                                                         mask=mask,
                                                                                                         truth_data=truth_trace_h_i,
                                                                                                         pred_data=pred_trace_h_i)
                trace_h_precision_list.append(float(precision_h))
                trace_h_recall_list.append(float(recall_h))
                trace_h_f1_list.append(float(f1_h))
